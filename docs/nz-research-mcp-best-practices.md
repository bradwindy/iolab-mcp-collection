...
- Auth options: none (public server — risky, anyone with the URL can use it and your upstream API keys), OAuth, or the newer request-header auth (e.g., an `x-api-key` header, in beta) — the latter is the pragmatic choice for a personal server.
