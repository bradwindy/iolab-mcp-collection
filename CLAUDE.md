...
- **`packages/mcp-kit`** — pagination, response formatting, TTL caching, rate-limit backoff, actionable
  ...
  bearer-auth middleware every server uses identically. Read the source before adding a tool — it's
...
`/mcp`, gated by one shared bearer token checked in `src/index.ts` *before* the request reaches the MCP
handler — deliberately not behind Cloudflare Access, since MCP clients connect server-to-server and
can't complete an interactive login. Tool handlers have the signature
`(rawInput: unknown, env: Env) => Promise<ToolTextResult>`, registered onto the agent's `McpServer` in
`src/agent.ts`.
