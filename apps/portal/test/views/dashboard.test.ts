...
function entry(overrides: Partial<ServerManifestEntry> = {}): ServerManifestEntry {
  return {
    slug: "nz-example-mcp",
    subdomain: "nz-example.mcp.iolab.nz",
    credentialKeys: [],
    ...overrides,
  };
}
...
describe("renderDashboard", () => {
  it("lists every server with its slug and subdomain URL", async () => {
    const rows: DashboardServerRow[] = [
      { entry: entry({ slug: "nz-geo-mcp", subdomain: "nz-geo.mcp.iolab.nz" }), statuses: [] },
      { entry: entry({ slug: "nz-govt-mcp", subdomain: "nz-govt.mcp.iolab.nz" }), statuses: [] },
    ];

    const body = String(await renderDashboard(rows));

    expect(body).toContain("nz-geo-mcp");
    expect(body).toContain("https://nz-geo.mcp.iolab.nz/mcp");
    expect(body).toContain("nz-govt-mcp");
    expect(body).toContain("https://nz-govt.mcp.iolab.nz/mcp");
  });
