...
describe("missingCredentialError", () => {
  it("points at the portal page for the specific server", () => {
    const result = missingCredentialError("nz-geo-mcp", "LINZ_API_KEY", "https://mcp.iolab.nz");
    expect(result.content[0]?.text).toBe(
      "The upstream API key 'LINZ_API_KEY' is not configured for nz-geo-mcp. Set it at https://mcp.iolab.nz/servers/nz-geo-mcp, then retry.",
    );
  });
});
